    ...
    <view class="catalog" :style="{'height' : '100%'}"  v-if="!loading">
        ...
        <scroll-view class="cate" scroll-y="true"  :style="{'height' : '100%'}">
            <view class="cate_item" v-for="( categoryChild , index1 ) of categoryList" :key="index1">
              ...
              <view class="bd">
                  <navigator hover-class="none" @click="$router.push({ path: '/pages/category/category', query: { Id: item.Id ,categoryChild: JSON.stringify(categoryChild.SubCategories) } })"  :class="(index2+1) % 3 == 0 ? 'last item' : 'item'" v-for="(item, index2) of categoryChild.SubCategories"
                      :key="item.Id">
