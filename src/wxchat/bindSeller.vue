    ...
    methods: {
        bind() {
            ...
            if(openId) {
                ...
                // 没有openId读取接口获取一次OpenId
                user.loginByWeixin().then(res => {
                   _this.bindSeller()
                }).catch((err) => {
                    _this.$wx.showErrorToast(err.msg)
                });
            }
        },
        async bindSeller() {
            // if(this.shopId == 0) {
            //     this.$wx.showErrorToast('网络异常，请重新扫描。')                
            //     return
            // }
            const openId = wx.getStorageSync('openId')
            var par = {
                ...
                ShopId: this.shopId,
                Name: this.userName,
                Type: this.serviceTypeGroup[this.index]
            }
            this.$wx.showLoading()
            const res = await api.addCustomerService(par)
            this.$wx.hideLoading()
            if(res.success) {
